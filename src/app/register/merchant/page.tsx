...
import { setMerchantProfile, updateMerchantBank, updateMerchantAvatar } from "@/store/userSlice";
...
export default function MerchantRegistrationPage() {
  ...
  const merchantProfile = useAppSelector((state) => state.user.merchant);
    ...
    fetch("/api/accounts")
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setMockAccounts(data.filter((acc) => acc.role === "merchant"));
        }
      })
      .catch((err) => {
  ...
  const handleGoogleCompleteSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(errs).length === 0 && googleUserInfo) {
      ...
      const generatedId = "KB-BUY-" + (form.phone ? form.phone.substring(6) : "5798") + "-" + cardIdSuffix;

      dispatch(setMerchantProfile({
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailOrId) {
      newLoginErrors.email = "Username/Corporate email or Buyer ID is required.";
    }
    ...
    try {
      const res = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({ email: emailOrId, password, role: "merchant" }),
      });
      ...
      if (data.user.bankDetails) {
        dispatch(updateMerchantBank(data.user.bankDetails));
      }
      if (data.user.avatar) {
        dispatch(updateMerchantAvatar(data.user.avatar));
      }
      ...
      <div className="w-full max-w-[850px] bg-white border border-gray-200/80 rounded-[24px] shadow-2xl relative overflow-hidden flex flex-col md:flex-row min-h-[500px] z-10">
                  ...
                  <div>
                    ...
                    <div className="mt-3 p-3.5 bg-emerald-50/60 border border-emerald-500/15 rounded-2xl flex flex-col gap-2.5 text-xs text-emerald-900">
                      <div className="font-extrabold text-[11px] text-emerald-850 uppercase tracking-wider flex items-center gap-1.5">
                        <span>💡</span> Select a Demo Merchant Account
                      </div>
        ...
        {isLoginMode && loginSuccess && (
          <div className="w-full p-8 sm:p-16 flex flex-col items-center text-center gap-6 bg-white text-gray-800">
            ...
            <div>
              <h2 className="text-2xl font-extrabold font-outfit text-[#1aa35a] mb-1">Welcome Back, {merchantProfile.name}!</h2>
            ...
            <div className="p-5 rounded-2xl border border-gray-200 bg-gray-50 text-left w-full max-w-[360px] flex flex-col gap-2.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-xs font-bold font-mono text-gray-800">{merchantProfile.id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-500 font-semibold">Region</span>
                <span className="text-xs font-bold text-gray-800">{merchantProfile.region}</span>
              </div>
              <div className="flex justify-between">
